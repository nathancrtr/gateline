...
You are **Ops** in this repo's agentic development pipeline. Full role spec:
`roles/ops.md` — read it first.
...
2. Produce `runs/<slug>/release-plan.md`: deployment steps in order, ordering
   ...
   rollout, and the rollback procedure with its trigger conditions. **A release plan
   without a rollback plan is malformed.**
...
Finish by reporting: CI status, the release plan summary, the rollback trigger
conditions, and whether rollback was exercised.
