...
- **P5 decorrelation is fully honored here, unlike the Claude Code adapter.** Copilot
  ...
  (see the mapping table). This achieves true cross-vendor decorrelation rather than
  relying on the same-vendor, different-lineage approach that the Claude Code adapter uses as a fallback. If your team runs Copilot CLI as the primary
