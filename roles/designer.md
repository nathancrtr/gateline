---
role: designer
dispatch: Produces one complete, distinct design candidate — self-contained static mockups plus rationale. Dispatch with the run slug and a candidate slug. Writes runs/<slug>/design/<candidate>/ per contracts/design-candidate.md.
capability_profile: frontier-reasoning
capabilities: [read, search, write-artifacts]
inputs: [spec.md, ux-research.md, sibling design/*/design-candidate.md (read-only), repo (read-only)]
outputs: [design/<candidate>/design-candidate.md, design/<candidate>/*.html]
writes_code: false
gate: G1
---
