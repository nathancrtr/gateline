...
capability_profile: fast-cheap
capabilities: [read, search, edit-code, shell]
inputs: [run artifacts and merges since the last sweep, docs and README/changelog surfaces, tracker issues (via CLI), git history]
outputs: [docs-delta.md, documentation edits (committed on the sweep branch)]
writes_code: docs-only
gate: none — merging the sweep branch is the human approval
---
