...
### ADR-8 (amendment — review-04.md F1, 2026-07-24): Task 04's surface widens to the harvest-then-dispose implementation surfaces; `base` is captured before the harness runs, and a fold failure never deletes the only surviving copy (R4, R5)
- **Choice:** Task 04's file-contact surface widens to `seam.ts` (`DispatchOutcome.harvest`), the new `harvest.ts` (`harvestPathspecs` — this plan's Interface contracts section claimed an existing private consumer in `engine.ts` to extract; none exists, corrected here), `workspace.ts` (`foldHarvestBranch`), `engine.ts` (`launch()`'s `managesOwnWorkspace` branch), and `runner-api.ts` (the outcome mirror) — the same widening pattern ADR-7 used for task 02. Two corrections landed on top after review-04.md's round-2 re-review of the first pass: the workspace's `base` (what the fold rebases onto) is captured immediately after `createWorkspace`, *before* the harness runs — not after, since every dispatch prompt (`prompts.ts`'s commit line) instructs the harness to commit its own work, and a `base` resolved post-harness makes a committing harness's work invisible to the harvest step (the round-1 defect, recurring through a different door — review-04.md F8). And `foldHarvestBranch` deletes the origin harvest branch only once the fold actually lands (`ok: true`), not on every exit path — a conflict or CAS exhaustion is exactly the case ADR-4 anticipates escalating to a human, and deleting the only surviving copy of the paid work on that path defeats the escalation (review-04.md F9).
- **Rejected:** Restricting the harvest to a `git add` sweep of uncommitted changes only (the first pass) — it silently drops any harvest-scoped file the harness committed itself, which is the norm this framework's own dispatch prompts establish, not the exception. Also rejected: deleting the origin harvest branch unconditionally in the fold's cleanup (simpler, but a failed fold then has no evidence for the human to recover from — the escalation ADR-4 designs for becomes irreversible instead of investigable).
- **Consequences:** No pending task's surface overlaps the widened set (task 05 is already `review-approved`; task 06 is untouched). `harvestAndPush`'s signature gains a `base` parameter the caller (task 04's `executeIntent`) must capture before invoking the harness — a caller that captures it late reintroduces F8 silently, so this ordering is load-bearing, not incidental. A conflicted or CAS-exhausted fold now leaves its origin harvest branch as a durable artifact until a human resolves the escalation and either re-derives or manually recovers it; nothing currently sweeps a resolved-but-abandoned harvest branch, a hygiene gap in the same family as F6 (leaked workstation clones). Widened under the G1 human's authority per review-04.md's resolution (direct implementation, no separate architect dispatch).
